...
In this chapter, we check whether a model's probabilities match observed outcomes and estimate its performance while we wait
for new labels. A loan model, for example, predicts repayment when someone applies, but we may wait months to learn whether
they repay. During that wait, we can estimate performance using earlier predictions and their outcomes.

Starting with a labeled \emph{reference dataset} from an earlier period, we estimate performance for each new batch of
predictions. In the figures, we call these batches \emph{chunks}. We keep the reference examples separate from the monitored
model's training data and validate the estimator on held-out data.

We call the agreement between probabilities and outcomes \emph{calibration} and measure it with Expected Calibration Error (ECE).
...
These estimates can track changes in the input mix, which we call \emph{covariate shift}. They assume the relationship between
inputs and outcomes stays the same. If that relationship changes, we have \emph{concept drift}. Once labels arrive, we use
Reverse Concept Drift (RCD) to estimate its effect on performance.}{In this chapter}
