...
We call those earlier predictions and outcomes a \emph{reference dataset}. Because the model did not train on these examples,
they show how it performs on unseen data. We use the reference dataset to estimate performance on new data, then compare the
estimates with actual performance on a separate set of labeled examples.

Before using probabilities to estimate performance, we need to check their \emph{calibration}: how well they agree with observed
outcomes. We start with Expected Calibration Error (ECE) for this check, then introduce CBPE and PAPE, two methods that use
calibrated probabilities to estimate classification metrics such as accuracy and F1. For regression, we cover Direct Loss
Estimation (DLE), which learns to predict the model's error.
