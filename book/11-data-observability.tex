...
The figures in this chapter follow one simulated example: an \texttt{orders} table with six weeks of purchases from an online shop. Each row records an order, with fields such as its amount, customer ID and arrival time. We introduce problems along the way, including incomplete loads, missing values and amounts stored in the wrong units, to see which monitors notice them.

We group each day's arrivals into a daily partition and calculate most metrics separately for each partition. The sum, for example, adds up that day's order amounts, while the average tells us the mean amount per order that day. Freshness checks the latest timestamp across the table, and schema monitoring checks its column names and types.

In the plots, gray bands show the expected ranges learned from earlier measurements, red points mark alerts, and hollow points indicate skipped or unavailable measurements. Following the same example across the chapter lets you compare what each monitor detects and what it misses.}{In this chapter}
...
Missing values percentage tells you what share of a column's values are null within the partition being checked, helping you check whether the information you need is present. A rise in missing country codes, for example, may indicate that a source field is no longer being loaded correctly.
...
Duplicate percentage shows what share of rows in a partition have a repeated key, such as an order ID used to identify a record. Here, we count every copy of a repeated key as a duplicate and leave out rows whose keys are null.
...
Unique count tells you how many different values appear in a column within a partition. It leaves out nulls and counts each value once, even if it appears in many rows. In our daily partitions, counting unique customer IDs tells you how many different customers appear in that day's load.
...
The average, or mean, is calculated within each partition: it adds up a column's non-null values and divides by how many there are. Here, it gives the average order amount for each day. Tracking the average can reveal a change in prices, quantities or other numeric data even when the row count stays the same.
...
The sum adds up the non-null values in a numeric column within each partition. In our example, it gives the total order amount for each day. A change in the total may come from a different number of rows, different values, or both.
...
Standard deviation measures how much numeric values vary around their average within a partition. Here, it tells us how widely order amounts vary within each day. A larger value means they are more spread out; zero means they are all the same. It is expressed in the same units as the original values.
...
Minimum and maximum are the smallest and largest non-null values in a column within a partition. Here, they give the smallest and largest order amounts for each day. They can reveal extremes that an average hides: a single negative quantity changes the minimum even if thousands of other quantities are valid.
...
Quartiles summarize the values within each partition, helping show where most lie without being dominated by a few extremes. Here, we calculate them separately for each day's order amounts. If you sort those values, Q1 marks one-quarter of the way through, the median marks halfway, and Q3 marks three-quarters of the way through.
...
Text length counts the characters in a value. Summarizing these lengths within each partition can reveal cut-off text, missing prefixes or changes in code format. Here, we track the average phone-value length for each day. Nulls are excluded, but an empty string has length zero.
