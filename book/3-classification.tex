...
\begin{center}
    \tikz{
        ...
        \draw[-latex, nmlpurple, semithick] ($(a.north)+(1.6, -0.5)$) to[bend right=20] node[pos=1, left] {Actual outcome} +(-1,.9);
        \draw[-latex, nmlcyan, semithick] ($(a.north)+(2.7,-0.5)$) to[bend left=15] node[pos=1, right] {Predicted probability} +(1,.9); 
        \draw[-latex, nmlred, semithick] ($(a.south)+(0.2,0.05)$) to[bend left=15] node[pos=1, left] {Number of samples} +(-1,-.5);
    }
\end{center}
