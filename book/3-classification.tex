...
particularly in segmentation tasks. It measures the overlap between the predicted set and the ground truth set. We can write the gneral formulation as:

\textbf{Jaccard Similarity} = \(\frac{\text{(number of observations in both sets)}}{\text{(number in either set)}}\)
        ...
        \node[inner sep=2pt, font=\Large] (a) {
            {
                ...
                {\textcolor{nmlred}{|A \cap B|} + \textcolor{nmlpurple}{|A \setminus B|} + \textcolor{nmlgreen}{|B \setminus A|}}
                $
            }
        };
        \draw[-latex, nmlred, semithick] ($(a.south)+(2, 1.2)$) to[bend right=35] node[pos=1, left] {\color{nmlred} Intersection (common elements)} +(-0.9, .7);
        \draw[-latex, nmlpurple, semithick] ($(a.south)+(1.8,0.04)$) to[bend left=35] node[pos=1, left] {\color{nmlpurple} Elements in A only } +(-.9, -.7);
        \draw[-latex, nmlgreen, semithick] ($(a.south)+(3.2,0.04)$) to[bend left=-35] node[pos=1, right] {\color{nmlgreen} Elements in B only } +(.9, -.7);
    }
\end{center}

where A and B are two sets of data.
        ...
        \node[inner sep=2pt, font=\Large] (a) {
            {
                $\displaystyle 
                {Jaccard\ Index} = 
                \frac{\textcolor{nmlred}{TP}}
                {\textcolor{nmlred}{TP} + \textcolor{nmlpurple}{FP} + \textcolor{nmlgreen}{FN}}
                $
            }
        };
        \draw[-latex, nmlred, semithick] ($(a.south)+(2, 1.2)$) to[bend right=35] node[pos=1, left] {\color{nmlred} true positive } +(-0.9, .7);
        \draw[-latex, nmlpurple, semithick] ($(a.south)+(1.8,0.04)$) to[bend left=35] node[pos=1, left] {\color{nmlpurple} false positive } +(-.9, -.7);
        \draw[-latex, nmlgreen, semithick] ($(a.south)+(3.2,0.04)$) to[bend left=-35] node[pos=1, right] {\color{nmlgreen} false negative } +(.9, -.7);
    }
\end{center}
