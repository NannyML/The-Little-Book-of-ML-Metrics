...
\begin{center}
    \tikz{
        \node[inner sep=2pt, font=\Large] (a) {
            {
                $\displaystyle
                MPD = \frac{1}{{\color{nmlred}n}} \sum_{t=1}^{n} 2 \left( {\color{nmlpurple}y_i} \log ( \frac {\color{nmlpurple}y_i}{\color{nmlcyan}\hat{\mu}_i} ) - ( {\color{nmlpurple}y_i} - {\color{nmlcyan}\hat{\mu}_i} ) \right)
                $
            }
        };
        \draw[-latex,nmlred, semithick] ($(a.south)+(-2.2,0)$) to[bend left=15] node[pos=1, left] {number of samples} +(-1,-1);
        \draw[-latex,nmlpurple, semithick] ($(a.north)+(0,0.0)$) to[bend left=15] node[pos=1, right] {observed count} +(1.5,.5); 
        \draw[-latex,nmlcyan, semithick] ($(a.south)+(1.3,0)$) to[bend right=15] node[pos=1, right] {predicted mean count} +(1.5,-1);
    }
\end{center}
