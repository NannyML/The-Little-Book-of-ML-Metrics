...
[, phantom, l sep=0pt, inner sep=0pt, for children={no edge}
  [{translation, or free-form generation against references}
    [{corpus-level $n$-gram precision with a brevity penalty} [{\mref{BLEU}{sec:bleu}}]]
    [{sentence-level; credits stems and synonyms; favors recall} [{\mref{METEOR}{sec:meteor}}]]
    [{how many edits would fix it} [{\mref{TER}{sec:ter}}]]]
  [{summarization}
    [{did the reference content make it in?} [{\mref{ROUGE}{sec:rouge}}]]]
  [{extractive question answering}
    [{is the short answer exactly right?} [{\mref{EM}{sec:em}}]]]
  [{speech transcription}
    [{how many words were substituted, dropped or added?} [{\mref{WER}{sec:wer}}]]]]
\end{forest}
