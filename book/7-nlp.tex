...
% ---------- Decision map ----------
\begin{decisionmap}{cyanhard}{cyanaccent}{Which metric?}{Start with the task. All six compare a candidate with reference text by surface overlap.}
\mapq{What is the task?}
\mapa{Translation, or free-form generation scored against references}{}
\begin{mapsub}
\mapq{What do you want to count?}
\mapa{$n$-gram precision over the corpus, with a brevity penalty}{\mref{BLEU}{sec:bleu}}
\mapa{Sentence-level matches that credit stems and synonyms, favoring recall}{\mref{METEOR}{sec:meteor}}
\mapa{The edits needed to turn the output into the reference}{\mref{TER}{sec:ter}}
\end{mapsub}
\mapa{Summarization: did the reference content make it in?}{\mref{ROUGE}{sec:rouge}}
\mapa{Extractive question answering: is the short answer exactly right?}{\mref{EM}{sec:em}}
\mapa{Speech transcription: how many words were substituted, dropped or added?}{\mref{WER}{sec:wer}}
