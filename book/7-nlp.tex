...
% ---------- Decision map ----------
\begin{decisionmap}{cyanhard}{cyanaccent}{Which metric?}{Follow the branches to a metric. The number next to it is its page.}
\maproot{What is the task?}
\begin{forest} dmap, large
[, coordinate, l sep=0pt
  [{\dm{translation, or generation against references}{What do you count?}}
    [{\dl{$n$-gram precision, over the corpus}{\mref{BLEU}{sec:bleu}}}]
    [{\dl{stems and synonyms too, per sentence}{\mref{METEOR}{sec:meteor}}}]
    [{\dl{the edits needed to reach the reference}{\mref{TER}{sec:ter}}}]
  ]
  [{\dl{summarization}{\mref{ROUGE}{sec:rouge}}}]
  [{\dl{extractive question answering}{\mref{EM}{sec:em}}}]
  [{\dl{speech transcription}{\mref{WER}{sec:wer}}}]
]
\end{forest}
\mapnotes{A correct paraphrase scores low on every metric here. When meaning matters more than wording, BERTScore (p.~\pageref{sec:bertscore}) is the reference-based option, and human judgment the final one.}
\end{decisionmap}
...
\begin{center}
    \tikz{
        ...
        \path[room for labels] ([yshift=0.21cm]a.north) ([yshift=-0.48cm]a.south);
        \draw[overlay,-latex,nmlred, semithick] ($(a.north)+(-1.35,-0.61)$) to[bend right=15] node[pos=1, left] {brevity penalty} +(-1.00,0.80);
        \draw[overlay,-latex,nmlcyan, semithick] ($(a.south)+(3.22,0.60)$) to node[pos=1, below] {n-gram precision} +(0.00,-0.79);
        \draw[overlay,-latex,nmlpurple, semithick] ($(a.south)+(1.75,0.69)$) to[bend left=15] node[pos=1, left] {n-gram weights} +(-1.00,-0.88);
    }
\end{center}
...
\clearpage
\thispagestyle{nlpstyle}
\section{METEOR}\label{sec:meteor}
...
\begin{center}
    \tikz{
        ...
        \path[room for labels] ([yshift=0.22cm]a.north) ([yshift=-0.21cm]a.south);
        \draw[overlay,-latex,nmlcyan, semithick] ($(a.north)+(2.91,-0.13)$) to[bend right=15] node[pos=1, left] {harmonic mean} +(-1.00,0.37);
        \draw[overlay,-latex,nmlpurple, semithick] ($(a.south)+(1.45,0.09)$) to[bend left=15] node[pos=1, left] {fragmentation penalty} +(-1.00,-0.30);
    }
\end{center}
...
\begin{center}
    \tikz{
        ...
        \path[room for labels] ([yshift=0.22cm]a.north) ([yshift=-0.16cm]a.south);
        \draw[overlay,-latex,nmlcyan, semithick] ($(a.north)+(2.34,-0.09)$) to[bend right=15] node[pos=1, left] {matching n-grams} +(-1.00,0.30);
        \draw[overlay,-latex,nmlpurple, semithick] ($(a.south)+(2.62,0.26)$) to[bend left=15] node[pos=1, left] {total reference n-grams} +(-1.00,-0.42);
    }
\end{center}
...
\begin{center}
    \tikz{
        ...
        \draw[overlay,-latex,nmlcyan, semithick] ($(a.north)+(-0.51,-0.04)$) to[bend right=15] node[pos=1, left] {word-level edits} +(-1.00,0.55);
        \draw[overlay,-latex,nmlpurple, semithick] ($(a.north)+(5.63,-0.03)$) to[bend right=15] node[pos=1, left] {sequence shifts} +(-1.00,0.35);
        \draw[overlay,-latex,nmlred, semithick] ($(a.south)+(2.11,0.04)$) to[bend left=15] node[pos=1, left] {words in reference} +(-1.00,-0.30);
    }
\end{center}
...
\begin{center}
    \tikz{
        ...
        \path[room for labels] ([yshift=0.38cm]a.north) ([yshift=-0.26cm]a.south);
        \draw[overlay,-latex,nmlred, semithick] ($(a.south)+(-0.71,0.34)$) to[bend left=15] node[pos=1, left] {number of samples} +(-1.00,-0.45);
        \draw[overlay,-latex,nmlpurple, semithick] ($(a.north)+(1.11,-0.61)$) to[bend right=15] node[pos=1, left] {predicted text} +(-1.00,0.85);
        \draw[overlay,-latex,nmlcyan, semithick] ($(a.south)+(2.16,0.60)$) to[bend right=15] node[pos=1, right] {reference text} +(1.00,-0.30);
    }
\end{center}
...
\begin{center}
    \tikz{
        ...
        \path[room for labels] ([yshift=0.30cm]a.north) ([yshift=-0.22cm]a.south);
        \draw[overlay,-latex,nmlcyan, semithick] ($(a.north)+(0.05,-0.01)$) to[bend right=15] node[pos=1, left] {edit operations} +(-1.00,0.30);
        \draw[overlay,-latex,nmlred, semithick] ($(a.south)+(0.99,0.05)$) to[bend left=15] node[pos=1, left] {words in reference} +(-1.00,-0.30);
    }
\end{center}
