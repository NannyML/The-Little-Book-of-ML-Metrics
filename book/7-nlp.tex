...
\textbf{Figure.} The candidate swaps two words, breaking every 4-gram, so the 4-gram precision $p_4$ is
...
\textbf{Figure.} One reference scored against three candidates. Replacing words with synonyms
...
\textbf{Figure.} A concise summary already covers the reference, so its ROUGE-1 recall is $0.78$.
...
\textbf{Figure.} The hypothesis is the reference with the phrase \textit{on monday morning} moved to the
...
\textbf{Figure.} Four predictions against the gold answer \textit{Martin Luther King}. Only the exact
...
\textbf{Figure.} The best alignment between reference and hypothesis splits the errors into one
